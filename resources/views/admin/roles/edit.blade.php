...
<div class="card">
    ...
    <p class="table-heading">{{ trans('global.edit') }} {{ trans('cruds.role.title_singular') }}</p>
    </div>

    <div class="card-body">
        <form action="{{ route("admin.roles.update", [$role->id]) }}" method="POST" enctype="multipart/form-data" class="row">
            @csrf
            @method('PUT')
