...
class PermissionRoleTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $company_admin_permissions = $superadmin_permissions->filter(function ($permission) {
            return (substr($permission->title, 0, 5) != 'user_' || $permission->title === 'user_management_access' ) && substr($permission->title, 0, 5) != 'role_' && substr($permission->title, 0, 11) != 'permission_' && substr($permission->title, 0, 4) != 'log_' && substr($permission->title, 0, 8) != 'refdata_' && substr($permission->title, 0, 13) != 'refdatafield_' && substr($permission->title, 0, 18) != 'doctype_management';
        });
